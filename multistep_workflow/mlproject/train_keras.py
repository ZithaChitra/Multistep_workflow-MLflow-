"""
Train a keras model for user/ratings. The input is a Parquet
ratings dataset (see etl_data.py) and an ALS model (see als.py),
which will use to supplement our input and training.
"""
...
def train_keras(ratings_data, als_model_uri, hidden_units):
	...
	train_mse = model.evaluate(x_train, pandas_df["rating"])
	test_mse = model.evaluate(x_test, pandas["rating"])
